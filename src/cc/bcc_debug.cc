...
void SourceDebugger::dumpSrcLine(const vector<string> &LineCache,
  ...
  if (Line != 0 && Line != CurrentSrcLine && Line < LineCache.size() &&
    ...
    errs() << "; "
          << StringRef(LineCache[Line - 1]).ltrim()
          << format(" // Line" "%4" PRIu64 "\n", Line);
    CurrentSrcLine = Line;
  }
}

void SourceDebugger::getDebugSections(StringMap<std::unique_ptr<MemoryBuffer>> &DebugSections) {
  for (auto section : sections_) {
    if (strncmp(section.first.c_str(), ".debug", 6) == 0) {
      ...
      DebugSections[section.first.substr(1)] = MemoryBuffer::getMemBufferCopy(SecData);
    }
  }
}
...
void SourceDebugger::dump() {
  ...
  MCInstPrinter *IP = T->createMCInstPrinter(TheTriple,
        0, *MAI, *MCII, *MRI);
  if (!IP) {
  ...
  std::unique_ptr<DWARFContext> DwarfCtx = DWARFContext::create(DebugSections, 8);
  if (!DwarfCtx) {
  ...
  for (auto section : sections_)
    if (!strncmp(fn_prefix_.c_str(), section.first.c_str(), fn_prefix_.size())) {
